...
function Emtra({ equipos, setEquipos, token }) {
  ...
  const [nuevoEquipo, setNuevoEquipo] = useState({
    ...
    fecha_asignacion: '', garantia: '', empresa_id: 2
  });
  ...
  const equiposEmtra = equipos?.filter(eq => eq.empresa_id === 2) || [];
  
  const equiposFiltrados = equiposEmtra.filter(eq => {
  ...
  const agregarEquipo = async () => {
    ...
    try {
      const res = await fetch(`${API_URL}/equipos`, {
        ...
        body: JSON.stringify({ ...nuevoEquipo, empresa_id: 2 })
      });
  ...
  const styles = {
    btnPrimary: { background: '#f39200', color: 'white', border: 'none', borderRadius: '8px', padding: '10px 24px', fontWeight: '600', cursor: 'pointer', fontSize: '14px' },
    btnSuccess: { background: '#28a745', color: 'white', border: 'none', borderRadius: '6px', padding: '6px 14px', fontWeight: '500', cursor: 'pointer', fontSize: '12px' },
    btnWarning: { background: '#f39200', color: 'white', border: 'none', borderRadius: '6px', padding: '6px 14px', fontWeight: '500', cursor: 'pointer', fontSize: '12px' },
    btnDanger: { background: '#dc3545', color: 'white', border: 'none', borderRadius: '6px', padding: '6px 14px', fontWeight: '500', cursor: 'pointer', fontSize: '12px' },
    btnSecondary: { background: '#6c757d', color: 'white', border: 'none', borderRadius: '6px', padding: '8px 18px', fontWeight: '500', cursor: 'pointer', fontSize: '13px' },
    btnBlue: { background: '#0a1b3d', color: 'white', border: 'none', borderRadius: '6px', padding: '6px 14px', fontWeight: '500', cursor: 'pointer', fontSize: '12px' },
  ...
  return (
    <div style={{ padding: '24px 32px', fontFamily: 'Segoe UI, Roboto, sans-serif', background: '#f0f2f5', minHeight: '100vh' }}>
      ...
      <div style={{ ...styles.card, display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '16px' }}>
        <div>
          <h1 style={{ fontSize: '24px', fontWeight: 600, color: '#1a1a2e', margin: 0 }}>Equipos EMTRA</h1>
          <p style={{ color: '#666', marginTop: '6px', fontSize: '14px' }}>
            Total: <strong style={{ fontSize: '20px', color: '#f39200' }}>{equiposEmtra.length}</strong> equipos registrados
          </p>
        </div>
        ...
        <div style={styles.card}>
          ...
          <div style={{ marginTop: '20px', display: 'flex', gap: '12px', justifyContent: 'flex-end', alignItems: 'center', flexWrap: 'wrap' }}>
            <input
              type="file"
              accept=".xlsx, .xls, .csv"
              id="import-excel-emtra"
              style={{ display: 'none' }}
              onChange={importarExcel}
            />
            <label htmlFor="import-excel-emtra" style={styles.btnSecondary}>
             Importar Excel
            </label>
      ...
      <div style={{ ...styles.card, padding: '16px 20px' }}>
        <div style={{ display: 'flex', gap: '12px', alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          <span style={{ fontSize: '12px', color: '#888', background: '#f0f2f5', padding: '4px 12px', borderRadius: '20px' }}>
            {busqueda ? `${equiposFiltrados.length} resultados` : `${equiposEmtra.length} total`}
          </span>
        </div>
      </div>
                  ...
                  <React.Fragment key={eq.id}>
                    ...
                    {equipoSeleccionado?.id === eq.id && !equipoEditando && (
                      <tr style={{ background: '#fef9f0' }}>
                        <td colSpan="8" style={{ padding: 15, border: '1px solid #aaa', borderTop: '2px solid #f39200' }}>
                          <div style={{
                    ...
                    {/* Editar equipo */}
                    {equipoEditando?.id === eq.id && tienePermiso(['super_admin', 'gestion']) && (
                      <tr style={{ background: '#fff8e7' }}>
                        <td colSpan="8" style={{ padding: '20px', borderTop: '2px solid #f39200' }}>
                          <h4 style={{ margin: '0 0 16px', color: '#f39200' }}>Editando: {eq.nombre_equipo}</h4>
            ...
            <div style={{ display: 'flex', gap: '4px' }}>
              {[...Array(totalPaginas)].map((_, i) => {
                ...
                if (Math.abs(numero - paginaActual) <= 2 || numero === 1 || numero === totalPaginas) {
                  return (<button key={numero} onClick={() => irPagina(numero)} style={{ padding: '6px 12px', borderRadius: '6px', border: '1px solid #dee2e6', backgroundColor: paginaActual === numero ? '#f39200' : 'white', color: paginaActual === numero ? 'white' : '#333', cursor: 'pointer', fontWeight: paginaActual === numero ? 'bold' : 'normal' }}>{numero}</button>);
        ...
        <div style={{
          ...
          <div style={{
            ...
            <p style={{ marginBottom: '24px', color: '#555', lineHeight: '1.5' }}>
              ...
              <strong style={{ color: '#f39200', fontSize: '16px' }}>{equipoAEliminar.nombre}</strong>
            </p>
